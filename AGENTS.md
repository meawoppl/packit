...
- When scripting checks, gate on the exact result line (e.g. `grep -q "^test result: ok. 12 passed"`), not on `set -e` through pipelines; piping test output through `grep`/`head` can hide a failure.
