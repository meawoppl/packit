...
- Frontend accounts: `frontend/src/account.rs` holds the session in an `AccountProvider` context (`Account { username, ask }`) and renders the header's `AccountMenu`. A page that needs an account emits `Account::ask` with a `done` callback; the play screen freezes its Share or Submit in memory (`Pending`), numbers each ask, and replays on a yes at most once. `frontend/src/webauthn.rs` bridges WebAuthn with `Reflect` (base64url to `ArrayBuffer` and back). Browser-test stubs for any API path (`Api`) and a fake `navigator.credentials` (`Passkeys`) live in `frontend/src/account/browser_tests.rs`; play-screen tests mount signed in with `mount_at`, or `mount_as(query, None)` signed out.
