...
(async()=>{
 ...
 try {
  ...
  const downloadPromise=page.waitForEvent('download');await page.locator('.pg-export').click();
  const exported=await downloadPromise;const report=JSON.parse(fs.readFileSync(await exported.path(),'utf8'));
  assert.deepEqual(report.algebraic.side_polynomial,['1','-4','7/2']);
  console.log('Exact branch:',report.algebraic.side_expression);
