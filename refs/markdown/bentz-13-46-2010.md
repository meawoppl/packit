...
**Corollary 3.** Let $0\lt a,b\leq1$ and $c$ such that both $c^2+b^2\leq1$ and $(a-c)^2+b^2\leq1$. Then if a box has it centre inside the rectangle $R$ with vertices $H(0,0)$, $I(0,b)$, $J(a,b)$, $K(a,0)$, it covers a vertex or the point $L(c,0)$. In particular, if $b\lt \frac12\sqrt3$, the box covers a vertex or the midpoint of the line segment $\overline{HK}$.
...
We will usually use Lemma 4 in the cases of $a\lt 2\sqrt2-2\approx0.828$, $b=1$ and $a=1$, $b\lt \sqrt2-\frac12\approx0.914$.

**Lemma 5 (Stromquist [8], [9]).** Let $2\sqrt2-2\lt a\lt 1$, $0\lt b\lt 1$, and $(a,b)$ within a distance of 1 from $(0,1)$. Moreover, let $f(a)$ be the infimum of
...
We will be using Lemma 5 in the cases $a=\frac12\sqrt3$, $b=0.5$; $a\leq0.89$, $0.6\lt b\leq0.921$; $a=0.90$, $b=0.90$; and $a=0.96$, $b=0.76$. Note that $f(a)$ is decreasing in $a$. (see original footnote 1 below)

**Original footnote 1:** In its original form the Lemma states incorrectly that the smallest positive $\theta$ satisfying (2) realizes the minimum of (1). In fact, the minimum values in some of our cases corresponds to the second largest positive root of (2).

**Lemma 6 (Stromquist [8]).** Let $L_1$ and $L_2$ be two parallel lines of distance $d\leq1$, and $B$ a box with its centre between them. Then $B$ must intersect the two lines with a common length of intersection of at least $\min\lbrace 1,2\sqrt2-2d\rbrace$.

We will use the Lemma in the situation of the following corollary, which will be a major technical tool for this article.

**Corollary 7.** Let $0\lt b\leq1$, and $R$ be the rectangle with vertices $(0,0)$, $(0,1)$, $(b,0)$, $(b,1)$. Then any box whose centre lies inside $R$ without containing any of its vertices intersects the line segments $\lbrace 0\rbrace \times[0,b]$ and $\lbrace 1\rbrace \times[0,b]$ with a common length of intersection of at least $2\sqrt2-2\approx0.828$. In addition, if $2\sqrt2-2>b$ then the box intersects each segment with a length of at least $b-2\sqrt2+2$.
...
Figure 1: Unavoidable set with 45 points (PDF page 4). [View the diagram.](../downloads/bentz-13-46-2010.pdf#page=4)
...
Figure 2: Initial configuration for 13 squares (PDF page 5). [View the diagram.](../downloads/bentz-13-46-2010.pdf#page=5)
...
Figure 3: One square has its centre in one of the marked regions (PDF page 6). [View the diagram.](../downloads/bentz-13-46-2010.pdf#page=6)
...
Figure 4: One box covers both marked points (PDF page 6). [View the diagram.](../downloads/bentz-13-46-2010.pdf#page=6)
...
```math
S_A=\lbrace A,(1.13,1),(1.4,1),(1.74,1),(1.87,0.76)\rbrace ,
```

```math
S_B=\lbrace B,(1,1.13),(1,1.82),(0.76,1.96)\rbrace
```

By Lemmas 10 and 11, the points in $S_A,S_B$ are covered by $T_A$ and $T_B$, respectively. Consider the set

```math
S_A\cup S_B\cup\lbrace (2.5,1),(3,1)\rbrace \cup(\lbrace 1,2,3\rbrace \times\lbrace 1.82,2.36,3.09\rbrace )
```

(see Figure 5). Non-avoidance lemmas apply to all regions with the exception of the four rectangular ones bounded by the points in $\lbrace 1,2,3\rbrace \times\lbrace 1.82,2.36,3.09\rbrace$. We will refer to this regions as $R_1,R_2,R_3$, and $R_4$, numbered clockwise from the top left.

Figure 5: Configuration with four critical regions (PDF page 7). [View the diagram.](../downloads/bentz-13-46-2010.pdf#page=7)
...
```math
\begin{aligned}
S_A\cup S_B\cup S_1\cup\lbrace &
(2.5,1),(3,1),(2,1.82),(3,1.82),\\
& (2.3,2.5),(3,2.5),(1,3),(2.3,3),(3,3)\rbrace
\end{aligned}
```

If we add an additional “sliding point” $Z$ on the line segment $\lbrace 1\rbrace \times[2.36,2.92]$, these points are non-avoidable regardless of the actual placement of $Z$ (see Figure 6; for $Z$ having a high $y$-value, this follows from Lemma 5 with $a\leq0.96$, $b=0.76$).

Figure 6: Case R₁, including a “sliding” point (PDF page 8). [View the diagram.](../downloads/bentz-13-46-2010.pdf#page=8)

Now by Corollary 7, $T'$ has a left intersection of at least $0.09$. Hence either $T'$ contains $(1,3)$ or intersects $\lbrace 1\rbrace \times[2.45,2.91]$. In both cases (with an appropriately placed $Z$), there are nine points which are not covered by $T_A,T_B$, or $T'$, and the set is small enough.
...
```math
\begin{aligned}
S_A\cup S_B\cup(S_2\cup\lbrace (2,2.72)\rbrace )\cup\lbrace &
(2.5,1),(3.1,1),(2.5,1.6),\\
& (1.6,1.95),(3.1,2),(1,2.72),(1,3),(1.8,3)\rbrace
\end{aligned}
```

Figure 7: Case R₂ (1.) (PDF page 9). [View the diagram.](../downloads/bentz-13-46-2010.pdf#page=9)
...
```math
\begin{aligned}
S_A\cup S_B\cup(S_2\cup\lbrace (3,2.64)\rbrace )\cup\lbrace &
(2.5,1),(3,1),(2,1.82),(3,1.82),\\
& (1,2.36),(1.5,2.36),(1,3),(1.8,3),(3,3)\rbrace .
\end{aligned}
```

Figure 8: Case R₂ (2.) (PDF page 10). [View the diagram.](../downloads/bentz-13-46-2010.pdf#page=10)
...
```math
\begin{aligned}
S_A\cup S_B\cup(S_3\cup\lbrace (3,2.10)\rbrace )\cup\lbrace &
(2.3,1),(3.05,1),(1.8,1.8),(3,1.86),\\
& (1,2.14),(2,2.14),(1,3),(1.5,3),(2.3,3),(3.1,3)\rbrace
\end{aligned}
```
...
Figure 9: Case R₃ (PDF page 10). [View the diagram.](../downloads/bentz-13-46-2010.pdf#page=10)
...
```math
\begin{aligned}
S_A\cup S_B\cup(S_4\cup\lbrace (1,2.10),(2,2.10)\rbrace )\cup\lbrace &
(2.5,1),(3.1,1),(2.4,1.6),\\
& (3.1,2),(2.4,2.5),(0.9,3),(1.7,3),(2.5,3),(3.1,3)\rbrace .
\end{aligned}
```
...
Figure 10: Case R₄ (PDF page 11). [View the diagram.](../downloads/bentz-13-46-2010.pdf#page=11)
...
- **Corollary 7, page 3:** the vertices describe $[0,b]\times[0,1]$, but the stated segments are $\lbrace 0\rbrace \times[0,b]$ and $\lbrace 1\rbrace \times[0,b]$. The additional condition is printed $2\sqrt2-2>b$, with the lower bound $b-2\sqrt2+2$; these are transcribed as printed.
...
- **Section 3.2.1, page 8:** “the following set of 18 points” is retained, although the displayed sets list 20 distinct points. Figure labels use typographic subscripts for the printed region numbers.
